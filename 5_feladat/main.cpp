...
int main(){
    ...
    sq_matrix<double> const add_res({1388,586,1303,320,1215,997,1049,940,1805,867,1225,1090,154,1377,512,198}, 4);
    ...
    sq_matrix<double> const sub_res({254,-560,-377,110,769,419,711,-916,143,-709,-97,568,36,-241,-386,100},4);
    ...
    sq_matrix<double> const prod_res1({865844,1012969,1094415,229647,1452336,1476176,1540000,991452,1087470,1696026,1576536,363407,241673,388772,284336,560823}, 4);
    ...
    sq_matrix<double> const prod_res2({1862058, 539055, 1247136, 840786, 722537, 747966, 511349, 329786, 2132556, 769174, 1467440, 774979, 1292948, 636842, 995560, 401915}, 4);
    ...
    sq_matrix<double> const mul1({1247.4, 1260.6, 1848, 231, 490.6, 635.8, 371.8, 2041.6, 1828.2, 1733.6, 1454.2, 574.2, 129.8, 1779.8, 987.8, 107.8}, 4);
    ...
    sq_matrix<double> div({5.67, 5.73, 8.4, 1.05, 2.23, 2.89, 1.69, 9.28, 8.31, 7.88, 6.61, 2.61, 0.59, 8.09, 4.49, 0.49}, 4);
    ...
    m19 *= 2.2;
    if(!sq_matrix_eq(m19, mul1, eps)){
        cout<<"Matrix multiplication error! (*=)"<<endl;
        cout<<m19<<endl;
        cout<<mul1<<endl;
    }

    sq_matrix<double> m20 = m2;
    m20 /= 100;
    if(!sq_matrix_eq(m20, div, eps)){
        cout<<"Matrix division error! (/=)"<<endl;
        cout<<m20<<endl;
        cout<<div<<endl;
    }

    sq_matrix<double> m21 = m1;
    m21 += m2;
    if(!sq_matrix_eq(m21, add_res, eps)){
        cout<<"Matrix addition error! (+=)"<<endl;
        cout<<m21<<endl;
        cout<<add_res<<endl;
    }

    sq_matrix<double> m22 = m1;
    m22 -= m2;
    if(!sq_matrix_eq(m22, sub_res, eps)){
        cout<<"Matrix substraction error! (-=)"<<endl;
        cout<<m22<<endl;
        cout<<sub_res<<endl;
    }

    sq_matrix<double> m23 = m1;
    m23 *= m2;
    if(!sq_matrix_eq(m23, prod_res1, eps)){
        cout<<"Matrix substraction error! (-=)"<<endl;
        cout<<m23<<endl;
        cout<<prod_res1<<endl;
    }
